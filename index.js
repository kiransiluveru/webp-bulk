...
const processDirectory = ({ extensions = [], inputFolder = __dirname, isOutputSeparate = false }) => {
    ...
    find.file(regEx, inputFolder, async (files) => {
        ...
        for (let i = 0, len = files.length; i < len; i++) {
            ...
            if (isOutputSeparate) {
                dir_of_file = path.resolve(__dirname, 'Output');
...
try {
    yargs.command('webp-bulk', " To convert images to webp format", () => {
